...
# ═══════════════════════════════════════════════════════════════════
# 2.  CPG Chunk Stepper  (used for BOTH training and inference)
# ═══════════════════════════════════════════════════════════════════

class CPGChunkStepper:
    """
    Chunk-based BDF integrator — the canonical spike generator.

    This is the single source of spike events for both training data
    generation and deployment on the Raspberry Pi.  Using the same
    integrator in both contexts eliminates any train/deploy mismatch
    that would arise from comparing a batch solve_ivp run (training)
    against a streaming chunk-based run (inference).

    Instead of calling solve_ivp once per timestep (expensive), this
    integrates the CPG over `chunk_size` timesteps in a single
    solve_ivp call, returning every intermediate point via t_eval.
    All threshold crossings within the chunk are detected and returned
    in order.

    Parameters
    ----------
    N           : int    number of CPG neurons (default 4)
    dt          : float  integration timestep (keep at 1.0)
    chunk_size  : int    number of dt steps per solve_ivp call
    spike_thresh: float  upward-crossing threshold for spike detection
    """
    ...
    def step_chunk(self):
        """
        Integrate one chunk of `chunk_size` timesteps.

        Returns
        -------
        spike_events : list of (t_spike, neuron_id) in time order
        final_vm     : (N,) float32  membrane potentials at end of chunk
        all_vm       : (chunk_size, N) float32  vm at every timestep
        """
        t_start = self.t
...
def run_cpg_chunked(N=4, tmax=80_000, cpg_start_time=5_000, chunk_size=50,
                    spike_thresh=-2.0):
    """
    Run the CPG via CPGChunkStepper and collect spike events + vm traces.

    This replaces the old `run_cpg` + `extract_spike_events` pair.
    The warm-up phase (0 → cpg_start_time) is run identically to how
    the deployed stepper will boot, ensuring the attractor state at
    the start of data collection is the same in training and inference.

    Parameters
    ----------
    N               : int   number of CPG neurons
    tmax            : int   total simulation end time (steps)
    cpg_start_time  : int   warm-up duration; data collection begins here
    chunk_size      : int   steps per solve_ivp call
    spike_thresh    : float upward-crossing threshold

    Returns
    -------
    spike_times     : (E,)    float32 — spike times in data-collection window
    spike_neurons   : (E,)    int32   — corresponding neuron indices
    vm_record       : dict with keys 't' and 'y' mimicking scipy OdeSolution,
                      for compatibility with plot_cpg_vm.
                      vm_record['t'] : (T,) float64 — time axis
                      vm_record['y'] : (N*4, T) float32 — state rows
                        rows 0,4,8,12 are vm for neurons 0-3 (others zero-padded)
    """
    cpg = CPGChunkStepper(N=N, chunk_size=chunk_size,
                          spike_thresh=spike_thresh)

    # ── Warm-up: match deployment boot sequence exactly ──────────
    warm_chunks = int(np.ceil(cpg_start_time / chunk_size))
    print(f"  Warming up CPG ({cpg_start_time} steps, "
          f"{warm_chunks} chunks of {chunk_size}) ...")
    for _ in range(warm_chunks):
        cpg.step_chunk()
    print(f"  CPG settled at t={cpg.t:.0f}\n")

    # ── Data collection: cpg_start_time → tmax ──────────────────
    collect_steps = tmax - cpg_start_time
    n_chunks      = int(np.ceil(collect_steps / chunk_size))

    all_spike_times   = []
    all_spike_neurons = []
    vm_t_list         = []   # time axis for plotting
    vm_y_list         = []   # (chunk_size, N) per chunk → stacked to (T, N)

    for _ in range(n_chunks):
        spike_events, _, chunk_vm = cpg.step_chunk()
        # chunk_vm : (chunk_size, N)

        # Collect spikes
        for t_sp, nid in spike_events:
            all_spike_times.append(t_sp)
            all_spike_neurons.append(nid)

        # Store vm for diagnostics — reconstruct approximate t axis
        # chunk_vm rows correspond to t = [prev_t+dt, ..., current_t]
        chunk_end = cpg.t
        chunk_start = chunk_end - chunk_vm.shape[0] * cpg.dt
        t_chunk = np.linspace(chunk_start + cpg.dt, chunk_end,
                               chunk_vm.shape[0])
        vm_t_list.append(t_chunk)
        vm_y_list.append(chunk_vm)   # (chunk_size, N)

    # ── Assemble spike arrays ────────────────────────────────────
    if len(all_spike_times) == 0:
        raise RuntimeError(
            "No spikes detected — check spike_thresh or increase tmax.")

    spike_times   = np.array(all_spike_times,   dtype=np.float32)
    spike_neurons = np.array(all_spike_neurons, dtype=np.int32)

    # Sort by time (should already be sorted, but guard against
    # floating-point ties at chunk boundaries)
    order         = np.argsort(spike_times, kind="stable")
    spike_times   = spike_times[order]
    spike_neurons = spike_neurons[order]

    print(f"  Total spike events : {len(spike_times)}")
    for i in range(N):
        print(f"    Neuron {i}: {(spike_neurons == i).sum()} spikes")

    # ── Assemble vm record (for plot_cpg_vm) ─────────────────────
    # Stack: vm_matrix shape (T_total, N)
    vm_matrix = np.concatenate(vm_y_list, axis=0)   # (T_total, N)
    t_axis    = np.concatenate(vm_t_list, axis=0)   # (T_total,)

    # Build a dict that mimics the scipy OdeSolution interface used
    # by plot_cpg_vm: sol.t and sol.y[i*4, :] for neuron i's vm.
    # Non-vm state variables (vf, vs, vus) are zero-padded since
    # only vm is needed for plotting and spike detection is already done.
    T_total = len(t_axis)
    y_full  = np.zeros((N * 4, T_total), dtype=np.float32)
    for i in range(N):
        y_full[i * 4, :] = vm_matrix[:, i]   # rows 0, 4, 8, 12

    vm_record = {"t": t_axis, "y": y_full}

    return spike_times, spike_neurons, vm_record
