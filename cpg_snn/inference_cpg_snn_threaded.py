...
def run_inference(cfg, onnx_path, out_dir,
    ...
    event_buf = SpikeEventBuffer(seq_len, N=4, n_gaits=n_gaits)
    ...
    # ── Main inference loop ───────────────────────────────────────
    n_chunks   = int(np.ceil(t_max / chunk_size))
    print(f"Running inference: {t_max} steps in {n_chunks} chunks "
    ...
    try:
        for chunk_idx in range(n_chunks):
            ...
            spike_events, final_vm, chunk_vm = cpg.step_chunk()
